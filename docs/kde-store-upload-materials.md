...
• Per-Code-Block Copy - Icon-only copy button at the lower-left of each code block
• Configurable Timeout - Set a custom response timeout for slow models (or disable it)
• Error Banners - Clear inline messages for network and timeout failures
...
- Configurable response timeout (Settings → Behavior); set to 0 to disable
- Separate Appearance and Behavior configuration tabs
- Inline error banners for network/timeout failures (dismissible)
